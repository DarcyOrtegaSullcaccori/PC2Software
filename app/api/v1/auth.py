...
from app.factories.ciudadano_factory import CiudadanoFactory
from app.factories.reciclador_factory import RecicladorFactory
from app.schemas.user import RecicladorCreate, TokenOut, UsuarioCreate, UsuarioOut

router = APIRouter()
ciudadano_factory = CiudadanoFactory()
reciclador_factory = RecicladorFactory()
...
def register_ciudadano(data: UsuarioCreate, db: Session = Depends(get_db)):
    ...
    data.rol = "ciudadano"
    return ciudadano_factory.crear(db, data)
...
def register_reciclador(data: RecicladorCreate, db: Session = Depends(get_db)):
    ...
    if data.dni and crud_user.get_by_dni(db, data.dni):
        raise HTTPException(status_code=400, detail="El DNI ya está registrado")
    return reciclador_factory.crear(db, data)
